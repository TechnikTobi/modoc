...
# ToDos:
########
# Umschalten der Schub-Einheiten aus Menü ausbauen (war früher für kg oder g gedacht)
# Konfigdateien von USB Stick auf Raspberry Speicher reinladen, wenn USB Stick angesteckt ist & Anleitung für Karl u Franz für Offline-Config erstellen
# Logging der Messwerte in einer CSV und periodisch auf USB Stick schreiben - auch noch beim Shotdown
# Option: Alle Meßwerte (ausser Tasten) in ein CSV File mitloggen (ev nicht bei jedem Sampling schreiben um USB Stick zu schonen)
# Neuer Menüpunkt: umschalten zw config_Modoc und config_Makerbeam oder ... istPruefstand     wieder einbauen lt e9h
# UPM Messdiagramm mit GasBox-Steuerung und Ausgabe auf PDF oder so
# Option: pro Messwert separate Glättung (Tiefe Ringbuffer)
# Option: automatisch nach einem Meßwert zb Drehzahl immer einen ScreenShot machen wenn innerhalb einer Testbeschreibung das Maximum überschritten Wird
...
# tobi6g / 2.5.2020:
# ------------------
# Wenn auf Raspberry dann für alle Files (config...) absoluten Pfad einbauen mit /home/sp da sonst er Autostart nicht funktioniert, da dort kein PATH= gesetzt werden kannObenWasWegstreichen
# Autostartprozedur hat mit e9h-Programmversion noch mit sudo python3.5 funktioniert - ab tobi6g startet es nur mehr mit python3.6 (ohne sudo)
#
# tobi6h / 3.5.2020:
# ------------------
# Kalibrieren mit Standardabweichung
# Mehrfaches Screenshot hintereinander vermeiden
# Temperaturwerte 0C (wenn Sensor nicht angesteckt ist) automatisch als ungültig erkennen und bei Graphen ausblenden
#
# tobi6i / 16. u 23.5.2020:
# ------------------
# Schub in N skalieren und anzeigen (Wunsch Franz Schaberger 2.5.2020)
# Temperaturwerte ca 400C (wenn Kabel angesteckt, aber kein Sensor verbunden ist) automatisch als ungültig erkennen und bei Graphen ausblenden
...
adcdirekt = 0                           # 1.. zeigt NUR die ADC Werte an
ADCinfoAnz = 1                          # 1.. zeigt ein Fenster an mit ADC Werten
istPruefstand = 1                       # 1.. Prüfstand / 0.. Makerbeam-Test-Aufbau bei SP
einheitSchub = 1                        # 1 .. N (Die Umschaltung der Einheiten war früher für kg u g ... ist obsolet)
History = 50
LowSamplingGrenze = 1
# logg = True
logg = False
...
class Messobjekt:
    ...
    def __init__(self, eingabeMesswertName, eingabeMesswertKurzname, eingabeMesswertEinheit, eingabeMinimalerMesswert, eingabeMaximalerMesswert, eingabeMinimalerYachsenWert, eingabeMaximalerYachsenWert, eingabeSkalenmax, eingabeSkalenIncLabel, eingabeSkalenIncStrich, eingabeIndexInDataStream, eingabeIstTagFarbe, eingabeIstNachtFarbe, eingabeGlättung, eingabeZugriffAufApp):

        self.MesswertName = eingabeMesswertName
        self.MesswertKurzName = eingabeMesswertKurzname
    ...
    def refreshYourValue(self, eingabeDataStream, eingabeModocKonstanten):  # wird pro Messobjekt aufgerufen, nimmt aus USB Stream den Arduino-Wert und Eicht,... speichert in .Value
        if self.IndexInDataStream < 5:
            ...
            if self.IndexInDataStream == 2: # Schub
                ...
                self.value = int((((eingabeDataStream[self.IndexInDataStream] / eingabeModocKonstanten["schub_eich1"] - eingabeModocKonstanten["schub_eich2"]) * einheitSchub) + eingabeModocKonstanten["schub_kal"]) * 0.00981)
...
class GraphenLinie:
    ...
    def __init__(self, eingabeCanvas, eingabeZugriffAufGraph, eingabePosX1, eingabePosX2, eingabePosY1, eingabePosY2, eingabeMessobjekt, eingabeGraphSamplingRate, eingabeIndexNr):
        self.meinCanvas = eingabeCanvas
    ...
    def updateAnzeige(self):
        ...
        # Temperaturwerte 0C (wenn Sensor nicht angesteckt ist) automatisch als ungültig erkennen und bei Graphen ausblenden
        # Temperaturwerte ca 400C (wenn Kabel angesteckt, aber kein Sensor verbunden ist) automatisch als ungültig erkennen und bei Graphen ausblenden
        if (self.meinMessobjekt.MesswertEinheit == "°C") & ((self.meinMessobjekt.value == 0 or self.meinMessobjekt.value > 390)):
            self.data = np.array([])
            if (self.linien.size > 1):
                self.meinCanvas.delete(self.einLinienLabel)
        else:
        ...
        if (self.linien.size > 1):
            self.meinCanvas.delete(self.einLinienLabel)
        if (self.linien.size > 0):
            self.einLinienLabel = self.meinCanvas.create_text(
                ( x2 + self.indexnr * int(fontSizeFaktor*60), y2 ),
                anchor = "nw",
                text=self.meinMessobjekt.MesswertKurzName,
                fill = self.farbeGraphenLinie,
...
class Dock:
    ...
    def __init__(self, eingabeRoot, eingabeApp, eingabeCanvas, eingabeDockHoehe, eingabeAbstandZumRand, eingabeADCinfoAnz):
        self.root = eingabeRoot
        ...
        self.ADCinfoAnz = eingabeADCinfoAnz
...
#            self.dockValues["TestbeschreibungsLabel"].place(    x = self.dockX1 + self.abstandZumRand + 950,   y = self.dockY1 + self.dockHoehe * 0.5, anchor = "sw")
    ...
    def restartCommand(self, eingabeUserName = "login"):
        print ("Programm nochmal starten mit Username: ", ['python3.6'] + [sys.argv[0]] + [eingabeUserName])
...
#            self.ADCAnzeigeFenster.toggle()
            if self.ADCinfoAnz:
                self.ADCinfoAnz = False
            else:
                self.ADCinfoAnz = True
            print (ADCinfoAnz, self.ADCinfoAnz)
...
#        if self.zugriffAufApp.fensterHoeheAufteilungWertegruppenRundinstrumente > 30:   ... SP Aenderung damit Wertegruppen kleiner werden koennen
    ...
    def eigentlicheKalibrieren_save(self):      # Stand gesichert SPR 3.5.2020
        ...
        toleranz = 100                  # Toleranz in Prozenz max Abweichung, wird pro Versuch erhöht
        ...
        while (versuchNr <= 20) & (bleibInWhileSchleife):       # bis zu x Versuche
            ...
            print("USBLensen ok ", istFehlerfrei," Kalibrieren-Versuch: ", versuchNr, "Messung: ", messungNr, "AnzahlKorr: ", anzahlKorrekterMessergebnisse, "Schub: ", schub, "Drehmoment: ", drehmoment)

            if messungNr == 5:              # es wird einigemale die USB ausgelesen, dann erst wird mal kontrolliert
                messungNr = 0
#                if (schub == vorherigerSchubWert) & (drehmoment == vorherigerDrehmomentWert):
                schubmin = (vorherigerSchubWert * 100/toleranz)
                schubmax = (vorherigerSchubWert * toleranz/100)
                drehmomentmin = (vorherigerDrehmomentWert * 100/toleranz)
                drehmomentmax = (vorherigerDrehmomentWert * toleranz/100)
                print("      Vorherige Werte: ", vorherigerSchubWert, vorherigerDrehmomentWert)
                print("      Toleranz: ", toleranz-100, "Schub: ", schub, " zw: ", schubmin, schubmax, "Drehmoment: ", drehmoment, " zw: ", drehmomentmin, drehmomentmax)
                if  (abs(schub) >= abs(schubmin)) & (abs(schub) <= abs(schubmax)) & (abs(drehmoment) >= abs(drehmomentmin)) & (abs(drehmoment) <= abs(drehmomentmax)):
                    # Aktueller Wert ist zwischen vorherigemWert - Toleranz in Prozent und vorherigemWert + Toleranz in Prozent
                    # Man muss immer die Absoluten Werte vergleichen sonst funkt bei negativen Werten das <= >=us nicht
                    ...
                    toleranz = toleranz * 1.1                     # beim nächsten Durchlauf nimm die Toleranz um x% größer, damit es bald passt

                if anzahlKorrekterMessergebnisse == 3:          # Wenn 3x HINTEREINANDER die Kontrolle OK war, dann fertig, sonst Abbruch und neuer Versuch
                    bleibInWhileSchleife = False

            self.anzeigeKalibrierungsErgebnisse.config(text = str(toleranz-100) + "% Tol " + str(versuchNr) + ". Versuch.. (" + str(anzahlKorrekterMessergebnisse) + ". Messwert) Drehmoment: " + str(vorherigerDrehmomentWert) + " / " + str(drehmoment) + "   Schub: " + str(vorherigerSchubWert) + " / " + str(schub))
            self.root.update()
    ...
    def eigentlicheKalibrieren(self):   # 3.5.2020 SPR: neue Idee: Standardabweichung bilden bei mehreren Versuchen
                                        # wenn die Standardabweichungen abnehmen zwischen den Versuchen, dann weitere Versuch
                                        # weil man annehmen kann, dass sich das System noch beruhigt
                                        # wenn die Standardabweichungen gleich bleiben, dann hat sich das System beruhigt -> fertig
        self.kalibrierenEnde()
        self.aktiverKalibrierungsvorgangFortsetzung = True
        ...
        mittelwertdrehmoment = 0
        abweichungdrehmoment = 0
        standardabweichungdrehmoment = 0
        vorherigestandardabweichungdrehmoment = 0

        mittelwertschub = 0
        abweichungschub = 0
        standardabweichungschub = 0
        vorherigestandardabweichungschub = 0
        ...
        while (versuchNr <= 20) & (bleibInWhileSchleife):       # bis zu x Versuche
            ...
            if messungNr > messungsMax:
                messungNr = 1
                mittelwertdrehmoment = mittelwertdrehmoment / messungsMax                       # der wirkliche Mittelwert
                mittelwertschub = mittelwertschub / messungsMax
                if mittelwertdrehmoment != 0:
                    standardabweichungdrehmoment = abweichungdrehmoment / mittelwertdrehmoment      # Die Standardabweichung
                else:
                    standardabweichungdrehmoment = 0
                if mittelwertschub != 0:
                    standardabweichungschub = abweichungschub / mittelwertschub
                else:
                    standardabweichungschub = 0
                print("     MW-Drehmoment (korrekt): ", mittelwertdrehmoment, " SAW-Drehmoment: ", standardabweichungdrehmoment,
                "     MW-Schub (korrekt): ", mittelwertschub, " SAW-Schub: ", standardabweichungschub)
                if (versuchNr > 1) & ((standardabweichungdrehmoment >= vorherigestandardabweichungdrehmoment) | (standardabweichungschub >= vorherigestandardabweichungschub)):
                    # mindestens 2 Versuche machen, aber wenn eines der beiden Messwerte nicht besser wird -> aufhören
                    bleibInWhileSchleife = False
                else:
                    versuchNr = versuchNr + 1

                    vorherigestandardabweichungdrehmoment = standardabweichungdrehmoment
                    mittelwertdrehmoment = 0
                    abweichungdrehmoment = 0

                    vorherigestandardabweichungschub = standardabweichungschub
                    mittelwertschub = 0
                    abweichungschub = 0

            else:
                mittelwertdrehmoment = mittelwertdrehmoment + drehmoment
                abweichungdrehmoment = abweichungdrehmoment + abs(vorherigerDrehmomentWert - drehmoment)   # Laufende Abweichung summierend
                vorherigerDrehmomentWert = drehmoment

                mittelwertschub = mittelwertschub + schub
                abweichungschub = abweichungschub + abs(vorherigerSchubWert - schub)   # Laufende Abweichung summierend
                vorherigerSchubWert = schub

                print("Kalibrieren-Versuch: ", versuchNr, "Messung: ", messungNr,
                "Drehmoment: ", drehmoment, " lfd MW-D: ", mittelwertdrehmoment, " lfd AW-D: ", abweichungdrehmoment)
                print("Kalibrieren-Versuch: ", versuchNr, "Messung: ", messungNr,
                "Schub: ", schub, " lfd MW-S: ", mittelwertschub, " lfd AW-S: ", abweichungschub)

                messungNr = messungNr + 1


            self.anzeigeKalibrierungsErgebnisse.config(text = str(versuchNr) + ". Versuch.. (" + str(messungNr) + ". Messwert) Drehmoment: " + str(vorherigerDrehmomentWert) + " / " + str(drehmoment) + "   Schub: " + str(vorherigerSchubWert) + " / " + str(schub))
            self.root.update()
    ...
    def userAnmeldung(self):
        ...
        if rasp:    # SPR 3.5.2020
            filesInDirectory = [file for file in os.listdir("/home/sp") if os.path.isfile(os.path.join("/home/sp", file))]
        else:
    ...
    def userAnmeldungFortsetzung(self):
        ...
        if self.aktiveAnmeldung:
            ...
            try: # Versuch die Datei zu öffnen
                ...
                configfilename = "/home/sp/config_" + userName + ".csv"
...
class ADCanzeige:
    ...
    def __init__(self, eingabeRoot, eingabeCanvas, eingabeText, istTag):
        ...
        self.text = eingabeText
        self.font = "Piboto"
        self.fontSize = int(fontSizeFaktor*int(self.meinCanvas.winfo_height()/20))
        self.label = None

        self.farbeADCBackground = None
        self.farbeADCForeground = None

        if istTag:
            self.farbeADCBackground = "grey10"
            self.farbeADCForeground = "red"
        else:
            self.farbeADCBackground = "grey90"
            self.farbeADCForeground = "orange"
    ...
    def zeichnen(self):
        self.label = Label(
            self.root,
            text = self.text,
            bg = self.farbeADCBackground,
            fg = self.farbeADCForeground,
            justify=LEFT,
            font = (self.font, int(fontSizeFaktor*self.fontSize))
        )
        self.label.pack()
...
class App:
    def __init__(self, eingabeRoot, eingabeCanvas, eingabeUserName, eingabeADCinfoAnz):
        ...
        self.MesswertObjekte = {
                                       #Name                 Kurzname,  Einheit     MinMesswert MaxMesswert MinYWert    MaxYWert    Skalenmax   SkalenIncLabel  SkalenIncStrich IndexInDataStream   IstTagFarbe IstNachtFarbe   Glättung
            "Drehzahl" :    Messobjekt("Drehzahl",             "DZ",    "U/m",      0,          20000,      0,          20000,      20,         10,             21,             0,                  "#fb3a36",  "#F44336",      self.Glättung,          self),
            "Drehmoment" :  Messobjekt("Drehmoment",           "DM",    "Ncm",      0,          400,        0,          400,        400,        10,             43,             1,                  "#9CCC65",  "#64DD17",      self.Glättung,          self),
            "Schub":        Messobjekt("Schub",                "SB",    "N",        0,          3000,       0,          3000,       3000,       6,              34,             2,                  "#53b9ff",  "#448AFF",      self.Glättung,          self),
            "Leistung":     Messobjekt("Leistung",             "LG",    "W",        0,          3000,       0,          3000,       3000,       6,              13,             3,                  "#ae00d3",  "#AA00FF",      self.Glättung,          self),
            "Gas":          Messobjekt("Gas",                  "GS",    "%",        0,          100,        0,          100,        100,        10,             21,             4,                  "#F09B41",  "#FB8C00",      self.Glättung,          self),

            "Temp1":        Messobjekt("Temperatur 1",         "T1",    "°C",        0,          400,        0,          300,        400,        16,             33,             9,                  "#e74173",  "#FF1744",      self.Glättung,          self),
            "Temp2":        Messobjekt("Temperatur 2",         "T2",    "°C",        0,          400,        0,          300,        400,        16,             33,             10,                 "#B2D753",  "#00b407",      self.Glättung,          self),
            "Temp3":        Messobjekt("Temperatur 3",         "T3",    "°C",        0,          400,        0,          300,        400,        16,             33,             11,                 "#82D7EC",  "#0091EA",      self.Glättung,          self),
            "Temp4":        Messobjekt("Temperatur 4",         "T4",    "°C",        0,          400,        0,          300,        400,        16,             33,             12,                 "#64FFC0",  "#18ad88",      self.Glättung,          self),
            "Temp5":        Messobjekt("Temperatur 5",         "T5",    "°C",        0,          400,        0,          300,        400,        16,             33,             42,                 "#F09B41",  "#FB8C00",      self.Glättung,          self),
            "Temp6":        Messobjekt("Temperatur 6",         "T6",    "°C",        0,          400,        0,          300,        400,        16,             33,             43,                 "#FF5090",  "#E91E63",      self.Glättung,          self),
            "Temp7":        Messobjekt("Temperatur 7",         "T7",    "°C",        0,          400,        0,          300,        400,        16,             33,             44,                 "#A070FF",  "#D500F9",      self.Glättung,          self),
            "Temp8":        Messobjekt("Temperatur 8",         "T8",    "°C",        0,          400,        0,          300,        400,        16,             33,             45,                 "#ae00d3",  "#AA00FF",      self.Glättung,          self),

            "VCC":          Messobjekt("VCC",                  "V+",    "V",        0,          20,         0,          20,         20,         16,             33,             27,                 "#00B0FF",  "#0091EA",      self.Glättung,          self),
            "TempCPU":      Messobjekt("TempCPU",              "TC",    "°C",       0,          100,        0,          100,        100,        16,             33,             14,                 "#90A4AE",  "#455A64",      self.Glättung,          self),
        ...
        self.dock = Dock(self.root, self, self.meinCanvas, self.dockHoehe, self.abstandZumRand, self.ADCinfoAnz)
        self.dock.zeichnen()
    ...
    def restartWegenLowSamplingRateProzedur(self):
        ...
        self.dock.restartCommand(userName)
    ...
    def appLoop(self):
        ...
        screenshotLast = time.time()

        ADCtext = ""
        if ADCinfoAnz:
            self.ADCAnzeigeFenster = ADCanzeige(
            self.root,
            self.meinCanvas,
            ADCtext,
            True
        ...
        while True:
            ...
            if ADCinfoAnz:
                ...
                # for key in self.MesswertObjekte:
                #     if key == "Drehmoment":
                #         ADCtext = ADCtext + ", " + str(self.meinUSBhandler.data)

                # 0.. Drehzahl, 1.. Drehmoment, 2.. Schub, 9.. Temp1
                ADCtext = "ADC- und Anzeigewerte.. \n"
                ADCin = self.meinUSBhandler.data[0]
                ADCfeld = "Drehzahl"
                ADCvalue = self.MesswertObjekte[ADCfeld].value
                ADCtext = ADCtext + "\n" + ADCfeld + ": \t\t" + str(int(ADCin)) + " \t\t--> " + str(ADCvalue)
                ADCin = self.meinUSBhandler.data[1]
                ADCfeld = "Drehmoment"
                ADCvalue = self.MesswertObjekte[ADCfeld].value
                ADCtext = ADCtext + "\n" + ADCfeld + ": \t" + str(int(ADCin)) + " \t\t--> " + str(ADCvalue)
                ADCin = self.meinUSBhandler.data[2]
                ADCfeld = "Schub"
                ADCvalue = self.MesswertObjekte[ADCfeld].value
                ADCtext = ADCtext + "\n" + ADCfeld + ": \t\t" + str(int(ADCin)) + " \t--> " + str(ADCvalue)
                ADCin = self.meinUSBhandler.data[9]
                ADCfeld = "Temp1"
                ADCvalue = self.MesswertObjekte[ADCfeld].value
                ADCtext = ADCtext + "\n" + ADCfeld + ": \t\t" + str(int(ADCin)) + " \t\t--> " + str(ADCvalue)
                ADCin = self.meinUSBhandler.data[10]
                ADCfeld = "Temp2"
                ADCvalue = self.MesswertObjekte[ADCfeld].value
                ADCtext = ADCtext + "\n" + ADCfeld + ": \t\t" + str(int(ADCin)) + " \t\t--> " + str(ADCvalue)

                self.ADCAnzeigeFenster.update(ADCtext)
                self.ADCAnzeigeFenster.zeichnen()

                # self.ADCAnzeigeFenster.delete()
...
#                    self.startZeitText = str(time.strftime("%H:%M:%S", time.localtime(time.time())))
                    screenshotAct = time.time()
                    if screenshotAct - screenshotLast > 2:
                        print("ScreenShot startbar: ", screenshotAct - screenshotLast)
                        self.dock.screenshotAusloesen()
                        screenshotLast = time.time()
                    else:
                        print("ScreenShot zu knapp: ", screenshotAct - screenshotLast)
...
class usbDataHandler:
    ...
    def __init__(self):
        ...
        self.simrechteck1lfd = 0
        self.simrechteck1 = 0
        self.simrechteck1Null = True
        self.simrechteck2lfd = 0
        self.simrechteck2 = 0
        self.simrechteck2Null = True
    ...
    def leseUSBline(self):
        ...
        if usbsim:
            ...
            self.simrechteck1lfd = self.simrechteck1lfd + 1
            if self.simrechteck1lfd == 33:
                self.simrechteck1lfd = 1
                if self.simrechteck1Null:
                    self.simrechteck1Null = False
                else:
                    self.simrechteck1Null = True
            if self.simrechteck1Null:
                self.simrechteck1 = 0
            else:
                self.simrechteck1 = random.randint(300, 330)

            self.simrechteck2lfd = self.simrechteck2lfd + 1
            if self.simrechteck2lfd == 88:
                self.simrechteck2lfd = 1
                if self.simrechteck2Null:
                    self.simrechteck2Null = False
                else:
                    self.simrechteck2Null = True
            if self.simrechteck2Null:
                self.simrechteck2 = 460  # 460 -> 395°C     465 -> 406°C
            else:
                self.simrechteck2 = random.randint(330, 340)

            self.simsteigend = self.simsteigend + 1
            if self.simsteigend > 1024:
                self.simsteigend = 0
            ...
            sim = np.array([
                ...
                5000, # ergibt mit modoc_conf_Modoc ca 4900 U/m

                # 1 Drehmoment
                #9000000+ random.randint(0, 10000),
                #simtobiValue,
                #random.randint(r_drehm, int(r_drehm * 1.2)),
                750000, # ergibt mit modoc_conf_Modoc ca 850 Ncm

                #2 Schub
                #600000+random.randint(0, 1000),
                #400000,
                #random.randint(r_schub, int(r_schub * 1.002)),
                10300000, # ergibt mit modoc_conf_Modoc und einheitSchub = 1 ca 27300 kg (was g sein sollten)
                ...
                # 10 .. temp2
                #random.randint(390,400),
                self.simrechteck2,

                # 11 .. temp3
                random.randint(323,340),
                #self.simsteigend,
...
if len(sys.argv) > 1:
    if os.path.isfile('/home/sp/config_' + sys.argv[-1] + '.csv'):
        userName =  sys.argv[-1]
    else:
        userName = "login"

app = App(root, windowCanvas, userName, ADCinfoAnz)
