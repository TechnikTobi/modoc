...
def SetDesign():
    ...
    if Design:  fRundinstument_Zeiger2 = "firebrick"
    else:       fRundinstument_Zeiger2 = "maroon"
...
#        self.EinLabelWert2 = Label(root, text=self.LabelText, bg=self.LabelBG, fg=self.LabelFG, font=("fixedsys", max(int(rrad[self.Nr]/8),10)))
        ...
        self.derStrich2 =        w.create_line(rxm[self.Nr]+math.cos(self.wi+wd/1.15)*(rzei[self.Nr]*0.1),           rym[self.Nr]+math.sin(self.wi+wd/1.15)*(rzei[self.Nr]*0.1),           rxm[self.Nr]+math.cos(self.wi+wd)*(rzei[self.Nr]-2),       rym[self.Nr]+math.sin(self.wi+wd)*(rzei[self.Nr]-2), fill=fRundinstument_Zeiger2, width=rzb[self.Nr])
        self.derStrich =         w.create_line(rxm[self.Nr]+math.cos(self.wi+wd*1.15)*(rzei[self.Nr]*0.1),           rym[self.Nr]+math.sin(self.wi+wd*1.15)*(rzei[self.Nr]*0.1),           rxm[self.Nr]+math.cos(self.wi+wd)*(rzei[self.Nr]-2),       rym[self.Nr]+math.sin(self.wi+wd)*(rzei[self.Nr]-2), fill=fRundinstument_Zeiger, width=rzb[self.Nr])
...
############################
    def DesignKeyIn(self):
        ...
        if Design == 1: GPIO.output(GasBoxpin, GPIO.HIGH)  # nur zum testen der Gaxboxsteuerung
        else:           GPIO.output(GasBoxpin, GPIO.LOW)
