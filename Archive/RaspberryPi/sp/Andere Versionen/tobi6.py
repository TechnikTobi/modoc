...
# ToDos:
########
# Graphen reparieren: wenn länger konstant, dann keine Scalierung
...
class Messobjekt:
    ...
    def __init__(self, eingabeMesswertName, eingabeMesswertEinheit, eingabeMinimalerMesswert, eingabeMaximalerMesswert, eingabeMinimalerYachsenWert, eingabeMaximalerYachsenWert, eingabeSkalenmax, eingabeSkalenIncLabel, eingabeSkalenIncStrich, eingabeIndexInDataStream, eingabeIstTagFarbe, eingabeIstNachtFarbe, eingabeGlättung):
    ...
    def refreshYourValue(self, eingabeDataStream, eingabeModocKonstanten):  # wird pro Messobjekt aufgerufen, nimmt aus USB Stream den Arduino-Wert und Eicht,... speichert in .Value
        if self.IndexInDataStream < 5:
            ...
            if self.IndexInDataStream == 1: # Drehmoment
                print(self.MesswertName + ": " + str(eingabeDataStream[self.IndexInDataStream]), end = "")
            ...
            if self.IndexInDataStream == 2: # Schub
                print("       " + self.MesswertName + ": " + str(eingabeDataStream[self.IndexInDataStream]))
...
class Hilfslinien:
    ...
    def zeichnen(self):
        ...
        print("HALLO")
        print(self.yAchseMinWert)
        print(self.yAchseMaxWert)
    ...
    def update(self, eingabePosX1, eingabePosX2, eingabePosY1, eingabePosY2, eingabeYachseMaxWert, eingabeYachseMinWert, eingabeAbstandZumRand):
        self.yAchseMaxWert = eingabeYachseMaxWert
        self.yAchseMinWert = eingabeYachseMinWert
        print(self.yAchseMinWert, self.yAchseMaxWert)
        self.posX1 = eingabePosX1
        self.posX2 = eingabePosX2
        self.posY1 = eingabePosY1
        self.posY2 = eingabePosY2
...
class GraphenLinie:
    ...
    def updateAnzeige(self):
        ...
        print(self.posY2 - max(0, min((self.data.item(0) - self.meinMessobjekt.MinimalerYachsenWert) / (self.meinMessobjekt.MaximalerYachsenWert - self.meinMessobjekt.MinimalerYachsenWert) * (self.posY2-self.posY1), (self.posY2-self.posY1))), end = "")
        print(" ", end = "")
        print(self.data.item(0), end = "")
        print(" ", end = "")
        print(self.meinMessobjekt.MinimalerYachsenWert, end = "")
        print(" ", end = "")
        print(self.meinMessobjekt.MaximalerYachsenWert)
...
class Dock:
    ...
    def __init__(self, eingabeRoot, eingabeApp, eingabeCanvas, eingabeDockHoehe, eingabeAbstandZumRand):
        ...
        self.menuPunkte = ["Zurück", "Kalibrieren", "Einheit Schub", "Glättung", "Horizontale Teilung", "Vertikale Teilung", "Tag/Nacht Wechsel", "Benutzer wechseln"]
    ...
    def zeichnen(self):
        ...
        if (self.aktiveAenderungHorizontaleTeilung == False) & (self.aktiveAenderungVertikaleTeilung == False):
            ...
            self.tastenLabels["1"].place(
                x = self.dockX1 + self.abstandZumRand + 900,
    ...
    def vertikaleTeilungNachLinks(self):
        if self.zugriffAufApp.fensterHoeheAufteilungWertegruppenRundinstrumente > 30:
...
class App:
    def __init__(self, eingabeRoot, eingabeCanvas, eingabeUserName):
        ...
        self.MesswertObjekte = {
            ...
            "Drehzahl" :    Messobjekt("Drehzahl",              "U/m",      0,          20000,      0,          20000,      20,         10,             21,             0,                  "#fb3a36",  "#F44336",      0),
            "Drehmoment" :  Messobjekt("Drehmoment",            "Ncm",      0,          400,        0,          400,        400,        10,             43,             1,                  "#9CCC65",  "#64DD17",      0),
            "Schub":        Messobjekt("Schub",                 "kg",       0,          3000,       0,          3000,       3000,       6,              34,             2,                  "#53b9ff",  "#448AFF",      0),
            "Leistung":     Messobjekt("Leistung",              "W",        0,          3000,       0,          3000,       3000,       6,              13,             3,                  "#ae00d3",  "#AA00FF",      0),
            "Gas":          Messobjekt("Gas",                   "%",        0,          100,        0,          100,        100,        10,             21,             4,                  "#F09B41",  "#FB8C00",      0),

            "Temp1":        Messobjekt("Temperatur 1",         "°C",        0,          400,        0,          300,        400,        16,             33,             9,                  "#e74173",  "#FF1744",      0),
            "Temp2":        Messobjekt("Temperatur 2",         "°C",        0,          400,        0,          300,        400,        16,             33,             10,                 "#B2D753",  "#00b407",      0),
            "Temp3":        Messobjekt("Temperatur 3",         "°C",        0,          400,        0,          300,        400,        16,             33,             11,                 "#82D7EC",  "#0091EA",      0),
            "Temp4":        Messobjekt("Temperatur 4",         "°C",        0,          400,        0,          300,        400,        16,             33,             12,                 "#64FFC0",  "#18ad88",      0),
            "Temp5":        Messobjekt("Temperatur 5",         "°C",        0,          400,        0,          300,        400,        16,             33,             42,                 "#F09B41",  "#FB8C00",      0),
            "Temp6":        Messobjekt("Temperatur 6",         "°C",        0,          400,        0,          300,        400,        16,             33,             43,                 "#FF5090",  "#E91E63",      0),
            "Temp7":        Messobjekt("Temperatur 7",         "°C",        0,          400,        0,          300,        400,        16,             33,             44,                 "#A070FF",  "#D500F9",      0),
            "Temp8":        Messobjekt("Temperatur 8",         "°C",        0,          400,        0,          300,        400,        16,             33,             45,                 "#ae00d3",  "#AA00FF",      0),

            "VCC":          Messobjekt("VCC",                   "V",        0,          20,         0,          20,         20,         16,             33,             27,                 "#00B0FF",  "#0091EA",      0),
            "TempCPU":      Messobjekt("TempCPU",               "°C",       0,          100,        0,          100,        100,        16,             33,             14,                 "#90A4AE",  "#455A64",      0),
            "arduvers":     Messobjekt("Arduino Version",       "",         0,          0,          0,          0,          0,          0,              0,              41,                 "#EEFF41",  "#ffff00",      0),
            "shutdown":     Messobjekt("shutdown",              "",         0,          0,          0,          0,          0,          0,              0,              35,                 "#000000",  "#000000",      0),
            "taste1":       Messobjekt("taste1",                "",         0,          0,          0,          0,          0,          0,              0,              36,                 "#000000",  "#000000",      0),
            "taste2":       Messobjekt("taste2",                "",         0,          0,          0,          0,          0,          0,              0,              37,                 "#000000",  "#000000",      0),
            "taste3":       Messobjekt("taste3",                "",         0,          0,          0,          0,          0,          0,              0,              38,                 "#000000",  "#000000",      0),
            "taste4":       Messobjekt("taste4",                "",         0,          0,          0,          0,          0,          0,              0,              39,                 "#000000",  "#000000",      0),
            #                                                                                                                           x,              y = a * x + (a-1)
        }
        ...
        print(self.WertegruppeEingabeArray)
        ...
        print(self.GraphenEingabeArray)
    ...
    def appLoop(self):
        ...
        while True:
            ...
            if self.dock.aktiveAnmeldung:
                ...
                if self.MesswertObjekte["taste1"].value == 0:                   # Kalibrieren
        ...
        if usbsim:
            ...
            try:
                ...
                if len(dataStream) != 48:
                    self.istFehlerfrei = False
                    print("USB DataStream Fehler: Länge ist " + str(len(dataStream)))
                else:
